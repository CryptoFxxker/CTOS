# -*- coding: utf-8 -*-
# ctos/drivers/bitget/driver_ccxt.py
# Bitget driver using ccxt library
# pip install ccxt
...
# ccxt connector
try:
    from ccxt import bitget as ccxt_bitget
except ImportError:
...
try:
    ...
    # 如果无法导入，使用备用方案
    def get_account(exchange='bitget', account='main'):
        return {
    ...
    def list_accounts(exchange='bitget'):
        return ['main', 'sub1', 'sub2']  # 默认账户列表

def get_bitget_credentials(account='main'):
    """获取Bitget账户认证信息"""
    try:
        account_config = get_account('bitget', account)
        return {
...
def get_account_name_by_id(account_id=0, exchange='bitget'):
    """
    根据账户ID获取账户名称
...
def init_bitget_clients(mode: str = "swap", api_key: Optional[str] = None, api_secret: Optional[str] = None, passphrase: Optional[str] = None, account_id: int = 0):
    """
    初始化ccxt Bitget客户端：
      mode = 'spot' 使用 bitget spot
      mode = 'swap' 使用 bitget futures
    ...
    if not (k and s and p):
        try:
            account_name = get_account_name_by_id(account_id, 'bitget')
            credentials = get_bitget_credentials(account_name)
            k = k or credentials.get('api_key', '')
            s = s or credentials.get('api_secret', '')
            p = p or credentials.get('passphrase', '')
            
            if k and s and p:
                print(f"从配置文件读取Bitget账户: {account_name} (ID: {account_id})")
        except Exception as e:
            print(f"从配置文件读取账户信息失败: {e}，尝试使用环境变量")
    
    # 如果配置文件没有，尝试环境变量
    if not (k and s and p):
        k = k or os.getenv("BITGET_API_KEY") or ""
        s = s or os.getenv("BITGET_API_SECRET") or ""
        p = p or os.getenv("BITGET_PASSPHRASE") or ""
    ...
    # 检查是否所有凭证都已设置
    if not (k and s and p):
        raise ValueError(f"Bitget API凭证未设置！请检查配置文件或环境变量。账户ID: {account_id}")
    ...
    config = {
        "apiKey": k,
        "secret": s,
        "password": p,  # Bitget使用passphrase作为password
...
class BitgetDriver(TradingSyscalls):
    """
    CTOS Bitget driver (ccxt connector).
    Mode-aware symbol normalization for Bitget style symbols:
    ...
    def __init__(self, bitget_client=None, mode="swap", default_quote="USDT",
                 price_scale=1e-8, size_scale=1e-8, account_id=0):
        self.cex = 'bitget'
        self.quote_ccy = 'USDT'
        self.account_id = account_id
        """
        :param bitget_client: Optional. An initialized ccxt exchange client.
                           If None, will try to instantiate Bitget client with defaults.
        ...
        if bitget_client is None:
            try:
                cli = init_bitget_clients(mode=mode, account_id=account_id)
                self.okx = cli["swap"] or cli["spot"]
                if self.okx:
                    print(f"✓ Bitget Driver初始化成功 (账户ID: {account_id})")
                else:
                    print(f"✗ Bitget Driver初始化失败 (账户ID: {account_id})")
                    self.okx = None
            except Exception as e:
                print(f"✗ Bitget Driver初始化失败 (账户ID: {account_id}): {e}")
                self.okx = None
        else:
            self.okx = bitget_client
            print(f"✓ Bitget Driver使用外部客户端 (账户ID: {account_id})")
    ...
    def _norm_symbol(self, symbol):
        """
        Normalize symbols to Bitget format.
        ...
        # Bitget format: spot uses BASE/USDT, swap uses BASE/USDT:USDT
        if self.mode == "swap":
    ...
    def fees(self, symbol='ETH/USDT:USDT', instType='SWAP', keep_origin=False):
        """
        获取资金费率信息。
        - 对于 Bitget，使用 fetch_funding_rate() 方法
        - 返回 (result, error)
        - 统一返回结构到"每小时资金费率"。
        """
        ...
        try:
            ...
            period_hours = 8.0  # Bitget默认8小时周期
    ...
    def place_order(self, symbol, side, order_type, size, price=None, client_id=None, **kwargs):
        """
        Normalize inputs to your bitget client.
        ...
        if not hasattr(self.okx, "create_order"):
            raise NotImplementedError("bitget client lacks place_order(...)")
    ...
    def revoke_order(self, order_id, symbol=None):
        if hasattr(self.okx, "cancel_order"):
            if not symbol:
                return False, ValueError("symbol is required for cancel_order on Bitget")
            full, _, _ = self._norm_symbol(symbol)
            try:
                resp = self.okx.cancel_order(id=order_id, symbol=full)
...
if __name__ == "__main__":
    driver = BitgetDriver(account_id=0)
