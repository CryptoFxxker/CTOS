# -*- coding: utf-8 -*-
# ctos/drivers/gate/driver_ccxt.py
# Gate driver using ccxt library
# pip install ccxt
...
# ccxt connector
try:
    from ccxt import gate as ccxt_gate
except ImportError:
    raise RuntimeError("请先安装ccxt: pip install ccxt")

# Import account reader
try:
    from configs.account_reader import get_account, list_accounts
except ImportError:
    # 如果无法导入，使用备用方案
    def get_account(exchange='gate', account='main'):
        return {
            'api_key': os.getenv("GATE_API_KEY", ""),
            'api_secret': os.getenv("GATE_API_SECRET", "")
        }
    
    def list_accounts(exchange='gate'):
        return ['main', 'sub1', 'sub2']  # 默认账户列表

def get_gate_credentials(account='main'):
    """获取Gate账户认证信息"""
    try:
        account_config = get_account('gate', account)
        return {
...
def get_account_name_by_id(account_id=0, exchange='gate'):
    """
    根据账户ID获取账户名称
...
def init_gate_clients(mode: str = "swap", api_key: Optional[str] = None, api_secret: Optional[str] = None, account_id: int = 0):
    """
    初始化ccxt Gate客户端：
      mode = 'spot' 使用 gate spot
      mode = 'swap' 使用 gate futures
    ...
    if not (k and s):
        try:
            account_name = get_account_name_by_id(account_id, 'gate')
            credentials = get_gate_credentials(account_name)
            k = k or credentials.get('api_key', '')
            s = s or credentials.get('api_secret', '')
            
            if k and s:
                print(f"从配置文件读取Gate账户: {account_name} (ID: {account_id})")
        except Exception as e:
            print(f"从配置文件读取账户信息失败: {e}，尝试使用环境变量")
    
    # 如果配置文件没有，尝试环境变量
    if not (k and s):
        k = k or os.getenv("GATE_API_KEY") or ""
        s = s or os.getenv("GATE_API_SECRET") or ""
    ...
    # 检查是否所有凭证都已设置
    if not (k and s):
        raise ValueError(f"Gate API凭证未设置！请检查配置文件或环境变量。账户ID: {account_id}")
...
class GateDriver(TradingSyscalls):
    """
    CTOS Gate driver (ccxt connector).
    Mode-aware symbol normalization for Gate style symbols:
      - spot:  "BASE/USDT"           e.g. "BTC/USDT"
      - swap:  "BASE/USDT:USDT"      e.g. "BTC/USDT:USDT" (ccxt永续合约格式)
    Accepts inputs like 'btc-usdt', 'BTC/USDT', 'BTC/USDT:USDT', 'btc', etc.
    """

    def __init__(self, gate_client=None, mode="swap", default_quote="USDT",
                 price_scale=1e-8, size_scale=1e-8, account_id=0):
        self.cex = 'gate'
        self.quote_ccy = 'USDT'
        self.account_id = account_id
        """
        :param gate_client: Optional. An initialized ccxt exchange client.
                           If None, will try to instantiate Gate client with defaults.
        :param mode: "swap" or "spot". If "swap", we use BASE/USDT:USDT format.
        ...
        if gate_client is None:
            try:
                cli = init_gate_clients(mode=mode, account_id=account_id)
                self.gate_client = cli["swap"] or cli["spot"]
                if self.gate_client:
                    print(f"✓ Gate Driver初始化成功 (账户ID: {account_id})")
                else:
                    print(f"✗ Gate Driver初始化失败 (账户ID: {account_id})")
                    self.gate_client = None
            except Exception as e:
                print(f"✗ Gate Driver初始化失败 (账户ID: {account_id}): {e}")
                self.gate_client = None
        else:
            self.gate_client = gate_client
            print(f"✓ Gate Driver使用外部客户端 (账户ID: {account_id})")
    ...
    def _norm_symbol(self, symbol):
        """
        Normalize symbols to Gate format.
        Returns (full_symbol, base_lower, quote_upper)
        Examples:
          _norm_symbol('btc') -> ('BTC/USDT:USDT', 'btc', 'USDT')  # in swap mode
          _norm_symbol('BTC/USDT:USDT') -> ('BTC/USDT:USDT', 'btc', 'USDT')
          _norm_symbol('SOL/USDT') -> ('SOL/USDT:USDT', 'sol', 'USDT')  # in swap mode
          _norm_symbol('BTC-USDT') -> ('BTC/USDT:USDT', 'btc', 'USDT')  # in swap mode
        """
        s = str(symbol or "").strip()
        if not s:
            return None, None, None

        # unify separators: convert - and _ to /
        s = s.replace("-", "/").replace("_", "/")
        
        # Handle ccxt swap format (BASE/QUOTE:QUOTE)
        if ":" in s:
            parts = s.split(":")
            base_part = parts[0].upper()
            if "/" in base_part:
                base, quote = base_part.split("/")
            else:
                base = base_part
            ...
            parts = s.split("/")
            base = parts[0].upper()
            quote = parts[1].upper() if len(parts) > 1 else self.default_quote
        else:
            # Only base provided
            base = s.upper()
            quote = self.default_quote

        # Gate format: spot uses BASE/USDT, swap uses BASE/USDT:USDT
        if self.mode == "swap":
            full = f"{base}/{quote}:{quote}"
        else:
            full = f"{base}/{quote}"
    ...
    def symbols(self, instType='SWAP'):
        ...
        :return: list[str]，如 ['BTC/USDT:USDT', 'ETH/USDT:USDT', ...]
        """
        if self.gate_client is None:
            # 兜底：无法从底层获取时，返回少量默认
            return ["BTC/USDT:USDT", "ETH/USDT:USDT", "SOL/USDT:USDT"] if str(instType).upper() == 'SWAP' else ["BTC/USDT", "ETH/USDT", "SOL/USDT"]

        try:
            markets = self.gate_client.load_markets()
            if self.mode == "spot":
                # 现货市场
    ...
    def exchange_limits(self, symbol=None, instType='SWAP'):
        ...
        :param symbol: 交易对符号，如 'BTC/USDT:USDT'，如果为None则返回全类型数据
        ...
        try:
            ...
            for symbol_name, market in markets.items():
                if self.mode == "spot" and market.get('type') != 'spot':
                    continue
                if self.mode == "swap" and market.get('type') != 'swap':
                    continue
    ...
    def fees(self, symbol='ETH/USDT:USDT', instType='SWAP', keep_origin=False):
        """
        获取资金费率信息。
        - 对于 Gate，使用 fetch_funding_rate() 方法
        - 返回 (result, error)
        - 统一返回结构到"每小时资金费率"。
        """
        if not hasattr(self.gate_client, 'fetch_funding_rate'):
            return {"symbol": symbol, "instType": "SPOT", "fundingRate_hourly": None, "raw": None}, None
        ...
        try:
            raw = self.gate_client.fetch_funding_rate(symbol=full)
            if keep_origin:
                return raw, None
            
            # ccxt返回格式: {'symbol': 'BTC/USDT:USDT', 'fundingRate': 0.0001, 'timestamp': 1692345600000, 'datetime': '2023-08-17T00:00:00.000Z'}
            fr_period = raw.get('fundingRate')
            ts_ms = raw.get('timestamp')
            period_hours = 8.0  # Gate默认8小时周期
    ...
    # -------------- market data --------------
    def get_price_now(self, symbol='ETH/USDT:USDT'):
        full, base, _ = self._norm_symbol(symbol)
        if hasattr(self.gate_client, "fetch_ticker"):
            try:
                data = self.gate_client.fetch_ticker(symbol=full)
                # ccxt返回格式: {'symbol': 'BTC/USDT:USDT', 'last': 2000.0, 'bid': 1999.0, 'ask': 2001.0, ...}
    ...
    def get_orderbook(self, symbol='ETH/USDT:USDT', level=50):
        full, _, _ = self._norm_symbol(symbol)
        if hasattr(self.gate_client, "fetch_order_book"):
            try:
                raw = self.gate_client.fetch_order_book(symbol=full, limit=int(level))
    ...
    def get_klines(self, symbol='ETH/USDT:USDT', timeframe='1h', limit=200):
        """
        Normalize to list of dicts:
        [{'ts': ts_ms, 'open': o, 'high': h, 'low': l, 'close': c, 'volume': v}, ...]
        """
        full, _, _ = self._norm_symbol(symbol)
        if not hasattr(self.gate_client, "fetch_ohlcv"):
            raise NotImplementedError("Public.fetch_ohlcv unavailable")

        try:
            # 拉取原始数据
            raw = self.gate_client.fetch_ohlcv(symbol=full, timeframe=timeframe, limit=int(limit))
        except Exception as e:
            return None, e
    ...
    def place_order(self, symbol, side, order_type, size, price=None, client_id=None, **kwargs):
        """
        Normalize inputs to your gate client.
        Expected mapping often is:
          place_order(symbol=..., side='buy'|'sell', type='market'|'limit', size=float, price=float|None, client_oid=...)
        """
        full, _, _ = self._norm_symbol(symbol)
        if not hasattr(self.gate_client, "create_order"):
            raise NotImplementedError("gate client lacks place_order(...)")
        ...
        try:
            ...
            order = self.gate_client.create_order(**params)
            ...
            # 将异常转换为字符串，确保可以序列化
            error_msg = str(e)
            # 如果是ccxt的ExchangeError，尝试提取更详细的错误信息
            if hasattr(e, 'args') and len(e.args) > 0:
                if isinstance(e.args[0], dict):
                    # 尝试提取错误详情
                    error_dict = e.args[0]
                    if 'label' in error_dict and 'message' in error_dict:
                        error_msg = f"{error_dict.get('label', '')}: {error_dict.get('message', '')}"
    ...
    def amend_order(self, order_id, symbol, price=None, size=None, side=None, order_type=None,
        ...
        try:
            ...
            if oerr is None:
                ...
                if od_id and str(od_id) == str(order_id):
                    existing_order = od
                else:
                    return None, None
            else:
        ...
        old_qty = _get(existing_order, ['amount', 'origQty', 'quantity', 'size', 'qty'])
        old_price = _get(existing_order, ['price', 'px'])
    ...
    def revoke_order(self, order_id, symbol=None):
        if hasattr(self.gate_client, "cancel_order"):
            if not symbol:
                return False, ValueError("symbol is required for cancel_order on Gate")
            full, _, _ = self._norm_symbol(symbol)
            try:
                resp = self.gate_client.cancel_order(id=order_id, symbol=full)
    ...
    def get_order_status(self, order_id, symbol=None, keep_origin=False):
        if not hasattr(self.gate_client, "fetch_order"):
        ...
        try:
            resp = self.gate_client.fetch_order(id=order_id, symbol=full)
            if keep_origin:
                if order_id is None:
                    return resp, None
    ...
    def get_open_orders(self, symbol=None, instType='SWAP', onlyOrderId=True, keep_origin=True):
        ...
        :return: (result, error)
        """
        if hasattr(self.gate_client, "fetch_open_orders"):
            try:
                if symbol:
                    try:
                        full, _, _ = self._norm_symbol(symbol)
                    ...
                    full = symbol
                resp = self.gate_client.fetch_open_orders(symbol=full)
                # print(resp)
                if onlyOrderId:
                    order_ids = []
                    # 兼容 list / dict 两种返回结构
    ...
    def cancel_all(self, symbol=None, instType='SWAP', order_ids=None):
        ...
        :return: (result, error)
        """
        if hasattr(self.gate_client, "cancel_all_orders"):
            try:
                if symbol:
                    try:
                        full, _, _ = self._norm_symbol(symbol)
                    ...
                    full = symbol
                resp = self.gate_client.cancel_all_orders(symbol=full)
                return resp, None
            except Exception as e:
                return None, e
        else:
    ...
    def fetch_balance(self, currency='USDT'):
        ...
        """
        if hasattr(self.gate_client, "fetch_balance"):
            try:
                # ccxt的fetch_balance不接受currency参数，返回所有币种余额
                raw = self.gate_client.fetch_balance()
    ...
    def get_position(self, symbol=None, keep_origin=False, instType='SWAP'):
        ...
        try:
            if hasattr(self.gate_client, "fetch_positions"):
                positions = self.gate_client.fetch_positions(symbols=[symbol] if symbol else None)
            else:
                return [], None
...
if __name__ == "__main__":
    driver = GateDriver(account_id=0)
    print(driver.get_price_now(symbol='ETH/USDT:USDT'))
